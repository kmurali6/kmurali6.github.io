...
var valueline = d3.line()
    ...
    .y(function(d){ return yScale(d.Life_Ladder);})
...
function load(){
    //d3.csv("world-happiness-report.csv").then(function(d){
    //    console.log(d);
    //});
	d3.csv("world-happiness-report.csv", function(d) {
	  return {
...
function drawChart(countryCode, countrylabel, color){
    ...
    return function(data){
        ...
        xScale.domain(d3.extent(data[1], function(d) { return d.Year; }));
        yScale.domain([0, 100]);
        ...
        tip = d3.tip().attr('class', 'd3-tip').offset([-5, 5]).html(function(d) {
            return "<strong style='color:" + color + "'>" + countryCode + " " + floatFormatValue(d.Life_Ladder)  + "</strong>"; 
        });   

        var path = innerChart.append("g").append("path")
        .attr("width", width).attr("height",height)
        .datum(data[1].map( (d, i) => {
            console.log("path : date", d.Year, "value", d.Life_Ladder);
            return {
                date : d.Year,
                value : d.Life_Ladder
            };
        ...
        innerChart.append("g").selectAll(".dot")
            ...
            .attr("cy", function(d) { return yScale(d.Life_Ladder) })
            .attr("r", 3)
            .call(tip)
...
function addCountriesList(data, i){
    ...
    d3.select("body")
        ...
        .enter()
        .append("option")
        .attr("Life_Ladder", function(d){ return d.id; })
        .text(function (d, i){return d.name;});

    d3.select("body").select("#country_select_container").select("select").on("change", function(){
        console.log(d3.select(this).property('Life_Ladder'));
        draw(
            d3.select(this).property('Life_Ladder'), 
            true,
            d3.select('input[name=type]:checked').node().Life_Ladder
        );
    });
}
